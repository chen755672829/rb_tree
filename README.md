...
## 二、本红黑树需要优化的地方：
  1、在删除节点时，本红黑树是找其节点的真后继，然后在红黑树上，让该节点与真后继做指针交换，以此类推，把删除的节点，移到树的叶子节点(没有算NULL节点)上，然后进行删除。我这里使用的是指针交换，感觉这里有些耗时，优化需要参考nginx中红黑树，进行优化。  
  2、该红黑树本来的目的不管理节点内存开辟与释放，让用户进行管理，但是销毁树时，需要使用后序遍历。我实在想不出来，销毁树，让用户销毁节点内存的后序遍历。  
  中型遍历：(1) dfs (2)可以使用栈模拟dfs (3) 利用找其后继的方式进行中序遍历。  
  后序遍历：（1）dfs（2）使用栈模拟dfs。目前只知道这两种，若有大佬知道其他方式进行后序遍历的话，**麻烦指导一下**。  
  所以目前销毁树，只能传入销毁节点的函数，在dfs时，进行销毁当前节点  
## 三、通过图片简单介绍一下红黑树
###  1、找节点前驱  
